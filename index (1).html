...
<head>
  ...
  <title>لوحة متابعة الفريق — أرامكو</title>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .member-card { background: var(--bg); border: 0.5px solid var(--border); border-radius: var(--radius-lg); padding: 1rem; text-align: center; }
    ...
    .type-btn { flex: 1; padding: 6px; border: 0.5px solid var(--border); border-radius: var(--radius); font-family: 'Cairo', sans-serif; font-size: 13px; cursor: pointer; background: var(--bg2); color: var(--text2); transition: all 0.2s; }
    ...
    .filter-btn { background: var(--bg2); border: 0.5px solid var(--border); border-radius: 20px; padding: 4px 14px; font-size: 12px; font-family: 'Cairo', sans-serif; color: var(--text2); cursor: pointer; transition: all 0.2s; }
    ...
    .close-btn { background:#E1F5EE; border:0.5px solid #9FE1CB; color:#0F6E56; border-radius:var(--radius); padding:4px 10px; font-size:12px; font-family:'Cairo',sans-serif; cursor:pointer; transition:all 0.2s; }
    ...
    .modal input { width:100%; padding:8px 12px; border:0.5px solid var(--border2); border-radius:var(--radius); font-family:'Cairo',sans-serif; font-size:14px; margin-bottom:8px; text-align:center; letter-spacing:4px; }
    ...
    /* Toast */
    .toast { position:fixed; bottom:24px; left:50%; transform:translateX(-50%) translateY(40px); background:#0F6E56; color:#fff; padding:8px 20px; border-radius:20px; font-size:13px; font-family:'Cairo',sans-serif; opacity:0; transition:all 0.3s; pointer-events:none; z-index:99; }
...
<div class="container">
  ...
  <div class="footer">Saudi Aramco · Team Task Tracker · 2025</div>
</div>
...
<div class="modal-backdrop" id="modal">
  <div class="modal">
    ...
    <p>أدخل كلمة مرور المدير لتتمكن من إغلاق التاسكات</p>
...
let currentType = "all", currentStatus = "all";
...
const tasks = [
  ...
  { id:2,  title:"Equipment Calibration",  owner:"Ahmed",   type:"CM", status:"pending"  },
  { id:3,  title:"Safety Audit",           owner:"Abdulla", type:"PM", status:"pending"  },
  ...
  { id:6,  title:"Valve Replacement",      owner:"Ahmed",   type:"CM", status:"progress" },
  ...
  { id:9,  title:"Asset Register Update",  owner:"Abdulla", type:"PM", status:"progress" },
  ...
  { id:12, title:"Bearing Replacement",    owner:"Ahmed",   type:"CM", status:"pending"  },
...
function updateAdminBar() {
  ...
  document.getElementById("status-text").textContent = isAdmin
    ? "مفتوح — بإمكانك إغلاق التاسكات"
    : "مقفل — تحتاج صلاحية المدير لإغلاق تاسك";
...
function tryCloseTask(id) {
  if (isAdmin) { doCloseTask(id); }
  else { pendingTaskId = id; openModal(); }
}
...
function updateStats() {
  ...
  const pmDone = tasks.filter(t => t.type === "PM" && t.status === "done").length;
  const cmDone = tasks.filter(t => t.type === "CM" && t.status === "done").length;
  const pct = Math.round(done / tasks.length * 100);
  document.getElementById("total-count").textContent = tasks.length;
  document.getElementById("done-count").textContent = done;
  document.getElementById("pm-done").textContent = pmDone;
  document.getElementById("cm-done").textContent = cmDone;
...
function renderTeam() {
  document.getElementById("team-grid").innerHTML = members.map(m => {
    ...
    const myPM = tasks.filter(t => t.owner === m.name && t.type === "PM" && t.status === "done").length;
    const myCM = tasks.filter(t => t.owner === m.name && t.type === "CM" && t.status === "done").length;
    ...
    return `<div class="member-card">
      ...
      <div style="font-size:12px;color:#5f5e5a;margin-top:6px;">
        <span style="color:#534AB7">PM: ${myPM}</span> &nbsp; <span style="color:#BA7517">CM: ${myCM}</span>
      </div>
